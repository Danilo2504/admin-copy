...
const profileImageUrl = 'https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/271deea8-e28c-41a3-aaf5-2913f5f48be6/de7834s-6515bd40-8b2c-4dc6-a843-5ac1a95a8b55.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..BopkDn1ptIwbmcKHdAOlYHyAOOACXW0Zfgbs0-6BY-E';
...
const UserProfile = ({ params }: any) => {
  const { data, isLoading, refetch, isRefetching }: UseQueryResult<UserProps> = useQuery({ 
    ...
    <div className='flex flex-col py-4 pl-2 pr-6 gap-6'>
      ...
      <div className='card-base flex flex-1 flex-col p-10'>
        <section className='flex gap-6'>
          <div>
            <img className='rounded-[50px]' src={profileImageUrl} width='80px' height='80px'/>
          </div>
          <div>
            <p className='text-[#344767] font-semibold'>
              {isLoading || isRefetching ? 'Loading...' : data?.fullName}
            </p>
            <p className='my-1'>{data?.email}</p>
            <button onClick={copyToClipboard} className='hover:text-[#3175DC] text-gray-500 inline-flex gap-2'>
          ...
          <div className='flex flex-1 gap-[6rem] mb-4'>
            <div className='flex flex-col'>
              ...
              <input
                ...
                name='name' 
                value={isLoading || isRefetching ? '...' : data?.fullName.split(' ')[0]}
                readOnly
              />
            </div>
            <div className='flex flex-col'>
              ...
              <input
                className='px-4 py-3 border-2 w-[375px] text-gray-500 border-solid rounded-xl border-gray-100'
                name='surname' 
                value={isLoading || isRefetching ? '...' : data?.fullName.split(' ')[1]}
                readOnly
              />
            </div>
          </div>
          ...
          <div className='flex flex-1 gap-[6rem]'>
            <div className='flex flex-col'>
              ...
              <input
                className='px-4 py-3 border-2 w-[375px] text-gray-500 border-solid rounded-xl border-gray-100'
                name='phone' 
                value={isLoading || isRefetching ? '...' : data?.cellPhone}
                readOnly
              />
            </div>
            <div className='flex flex-col'>
              ...
              <input
                className='px-4 py-3 border-2 w-[375px] text-gray-500 border-solid rounded-xl border-gray-100'
                name='email' 
                value={isLoading || isRefetching ? '...' : data?.email}
                readOnly
              />
            </div>
          </div>
