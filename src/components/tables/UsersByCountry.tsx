...
const UsersByCountryTable = () => {
  ...
  const { data, isPending }: UseQueryResult<UsersByCountry[]> = useQuery({
  ...
  return (
    <div className="card-base px-4 h-min sticky top-4">
      <h2 className="text-[#344767] font-semibold py-2 mb-4">Ranking</h2>
      ...
      <table className="w-full text-sm text-left text-gray-500">
        ...
        <UserByCountrySkeleton length={5} isVisible={isPending} />
        {!isPending && (
          <tbody>
            {data?.map((row: UsersByCountry) => (
              ...
              >
                ...
                <td className="p-4">{row._count}</td>
              </tr>
            ))}
          </tbody>
        )}
